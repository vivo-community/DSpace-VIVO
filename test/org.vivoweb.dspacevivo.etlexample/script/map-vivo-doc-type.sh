...
inputGraph=$(realpath $1)
###
# Remove   trim leading and trailing whitespace plus " caracter to docValue
docValues="$(grep terms/type $inputGraph | cut -d ' ' -f 3- | tr -d '.' |  func-encode_string_to_i18n_lowercase.sh | tr -d '\"' | func-clean-begin-ending-whitespace.sh)" 
while read docValue; do 
    VIVO_URI=$(grep "\"$docValue\"" < $MAPPING_DATA_DIR/dspace_vivo_url_mapping_list.data | cut -d '"' -f 3 | xargs)
